...
export default function Layout(props: ParentProps) {
    ...
    if (!newDesign())
      Array.from({ length: 9 }, (_, i) => {
        ...
        commands.push({
          ...
          keybind: `mod+${number}`,
          disabled: (layout.projects.list()?.length ?? 0) <= index,
          hidden: true,
  ...
  const SidebarPanel = (panelProps: {
    ...
    const empty = createMemo(() => !params.dir && (layout.projects.list()?.length ?? 0) === 0)
    ...
    <Show
      ...
      fallback={
        <div id="root-layout" class="relative bg-v2-background-bg-deep flex-1 min-h-0 min-w-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text" style={appStyle()}>
          ...
          {settings.general.showDebugBar() && <DebugBar />}
          <HelpButton />
