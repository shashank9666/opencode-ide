...
export default function Page(props: { sessionId?: string; dir?: string; embedded?: boolean }) {
  ...
  const composer = createSessionComposerState({ sessionId: () => activeSessionId() })
  ...
  createEffect(
    on(
      () => ({ dir: activeDir(), id: activeSessionId() }),
      (next, prev) => {
        if (!prev) return
  ...
  const [sessionSync] = createResource(
    () => [sdk().directory, activeSessionId()] as const,
    ([directory, id]) => {
      ...
      ([dir, id, status, blocked]) => {
        ...
        todoFrame = requestAnimationFrame(() => {
          ...
          todoTimer = window.setTimeout(() => {
            todoTimer = undefined
            if (sdk().directory !== dir || activeSessionId() !== id) return
            untrack(() => {
  ...
  createEffect(
    on(
      () => sync().data.session_status[activeSessionId() ?? ""]?.type,
      (next, prev) => {
  ...
  fill = () => {
    ...
    fillFrame = requestAnimationFrame(() => {
      ...
      if (!activeSessionId() || !messagesReady()) return
  ...
  const restore = (id: string) => {
    if (!activeSessionId() || reverting()) return
  ...
  const composerRegion = (placement: "dock" | "inline") => (
    <SessionComposerRegion
      ...
      followup={
        activeSessionId() && !isChildSession()
          ? {
              queue: queueEnabled,
              items: followupDock(),
              ...
              onSend: (id) => {
                void sendFollowup(activeSessionId()!, id, { manual: true })
              },
              onEdit: editFollowup,
      ...
      <Show
        ...
        fallback={
          <>
            ...
            >
              <Show when={!isDesktop() && !!activeSessionId()}>
              ...
              >
                <div
                  classList={{
                    ...
                    "shadow-[var(--v2-elevation-raised)]": settings.general.newLayoutDesigns() && !!activeSessionId(),
                  }}
                >
                  <div class="flex-1 min-h-0 overflow-hidden">
                    <Switch>
                      <Match when={activeSessionId() && mobileChanges()}>
