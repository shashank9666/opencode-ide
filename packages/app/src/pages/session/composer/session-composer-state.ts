...
export function createSessionComposerState(options?: { closeMs?: number | (() => number); sessionId?: () => string | undefined }) {
  ...
  const id = createMemo(() => (options?.sessionId ? options.sessionId() : params.id))

  const questionRequest = createMemo((): QuestionRequest | undefined => {
    const currentId = id()
    return sessionQuestionRequest(sync().data.session, sync().data.question, currentId)
  })

  const permissionRequest = createMemo((): PermissionRequest | undefined => {
    const currentId = id()
    return sessionPermissionRequest(sync().data.session, sync().data.permission, currentId, (item) => {
  ...
  const todos = createMemo((): Todo[] => {
    const currentId = id()
    if (!currentId) return []
    return serverSync().data.session_todo[currentId] ?? []
  })
  ...
  const live = createMemo(() => sync().data.session_working(id() ?? "") || blocked())
  ...
  const clear = () => {
    const currentId = id()
    if (!currentId) return
    serverSync().todo.set(currentId, [])
    sync().set("todo", currentId, [])
  }
