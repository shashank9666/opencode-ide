...
function HomeDesign() {
  ...
  function openIde(conn: ServerConnection.Any, directory: string) {
    const ctx = global.createServerCtx(conn)
    ctx.projects.open(directory)
    ctx.projects.touch(directory)
    navigateOnServer(conn, `/${base64Encode(directory)}/ide`)
  }
  ...
  const selectedServer = createMemo(() => {
    ...
    if (!conn) return
    const ctx = global.createServerCtx(conn)
    const project = selectedProject() ?? ctx.projects.list()[0]
    if (!project) return
    return conn
  })
    ...
    <div class="rounded-[10px] shadow-[var(--v2-elevation-raised)] m-2 min-h-0 lg:overflow-hidden bg-v2-background-bg-base self-stretch flex-1">
      <div class="mx-auto grid w-full h-full max-w-[1080px] gap-8 px-6 pb-16 lg:grid-cols-[280px_minmax(0,720px)]">
        <HomeProjectColumn
          ...
          openIde={(conn, directory) => openIde(conn, directory)}
...
function HomeProjectColumn(props: {
  ...
  openIde: (conn: ServerConnection.Any, directory: string) => void
  ...
  return (
    <aside class="flex min-w-0 flex-col lg:pt-[52px] mt-14 gap-4" aria-label={props.language.t("home.projects")}>
      ...
      <div class="mt-4 flex min-w-0 flex-col gap-1">
        ...
        <Show when={props.selectedServer && props.selectedDirectory}>
          <button
            type="button"
            class={`${HOME_PROJECT_NAV_ROW} text-v2-text-text-faint [&>[data-slot=icon-svg]]:text-v2-icon-icon-muted`}
            onClick={() => props.openIde(props.selectedServer!, props.selectedDirectory!)}
          >
            <IconV2 name="window" size="small" />
